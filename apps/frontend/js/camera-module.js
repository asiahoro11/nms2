...
function cameraLicenseRequiredText() {
    return camText('cameras.license_required', '請先啟用攝影機授權');
}
...
function applyLicenseUI() {
    ...
    if (cameraLicensed) {
        ...
        if (addBtn) {
            ...
            addBtn.title = '';
            addBtn.innerHTML = `＋ <span data-i18n="cameras.add">${escapeHtml(camText('cameras.add', '新增攝影機'))}</span>`;
        }
        discoverBtns.forEach(b => { b.disabled = false; b.classList.remove('btn-disabled'); });
    } else {
        const licenseText = cameraLicenseRequiredText();
        if (notice) notice.style.display = 'flex';
        if (addBtn) {
            addBtn.disabled = true;
            addBtn.classList.add('btn-disabled');
            addBtn.title = licenseText;
            addBtn.innerHTML = `&#128274; <span data-i18n="cameras.add">${escapeHtml(camText('cameras.add', '新增攝影機'))}</span>`;
        }
    ...
    if (typeof setModuleLock === 'function') {
        setModuleLock('camera', !cameraLicensed);
    } else {
...
// Called from img onerror. Retries with exponential backoff (3s→8s→2s, max 30s).
...
window.discoverCameras = async function () {
    if (!cameraLicensed) {
        if (typeof showToast === 'function') showToast(cameraLicenseRequiredText(), 'warning');
        return;
    }
    ...
    openModal('\u65b0\u589e\u651d\u5f71\u6a5f', `
        <form id="camera-add-form" onsubmit="window.handleCameraSubmit(event)">
            <div class="form-group" style="margin-bottom:12px;">
                ...
                <small class="text-muted">${camText('cameras.connection_mode_hint', '\u652f\u63f4\u76f4\u63a5 RTSP \u6216 ONVIF \u81ea\u52d5\u63a2\u6e2c')}</small>
            </div>
...
// ─── ONVIF probe ────────────────────────────────────────────────────────────
// mode: 'add' (Add modal) | 'edit' (Edit modal)
